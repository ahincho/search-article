...
\begin{enumerate}
    ...
    \item \textbf{Eficiencia superior:} La incorporación de información heurística reduce significativamente el espacio de búsqueda explorado. La reducción oscila entre 12\% en grids con movimiento cardinal y 97\% en grids grandes ($50 \times 50$), demostrando que la ventaja de A* se amplifica con la escala del problema. En el grafo de ciudades, A* explora un 50\% menos de nodos que Dijkstra.
...
\begin{enumerate}
    ...
    \item \textbf{Jump Point Search:} Harabor y Grastien \cite{harabor2011online} demuestran que JPS logra aceleraciones de un orden de magnitud sobre A* en grids uniformes mediante la eliminación de simetrías, representando el estado del arte para pathfinding en grids estáticos.
    
    \item \textbf{Any-Angle Pathfinding:} Nash et al. \cite{nash2010theta} evidencian que los caminos restringidos a la topología del grid son subóptimos respecto al espacio continuo (hasta 76\% más largos). Theta* aborda esta limitación permitiendo trayectorias en ángulo libre.
    
    \item \textbf{Anytime Search:} Hansen y Zilberstein \cite{hansen2001anytime} formalizan Weighted A* ($f(n) = g(n) + \varepsilon \cdot h(n)$, $\varepsilon > 1$) como marco para búsqueda con restricciones de tiempo, aplicable a robótica y videojuegos en tiempo real.
    
    \item \textbf{Entornos dinámicos:} Extender la librería para soportar replanificación en tiempo real ante cambios en la topología o costos del grafo mediante D* Lite \cite{koenig2002d}.
    
    \item \textbf{Paralelización:} Explorar variantes paralelas de A* (HDA*, PRA*) para aprovechar arquitecturas multi-core \cite{fukunaga2017survey}.
