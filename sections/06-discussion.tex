...
En robótica móvil, A* se aplica para la planificación de trayectorias en mapas de ocupación (occupancy grids) \cite{lavalle2006planning}. Kabir et al. \cite{kabir2024enhanced} demuestran que A* con una función de costo adaptada a las restricciones cinemáticas del robot reduce el tiempo de planificación un 51.96\% respecto a Dijkstra en simulaciones ROS con Turtlebot3. La clase \texttt{Grid} con costos variables puede representar terrenos con diferente dificultad de traversal, mapeando directamente a estos entornos de navegación robótica.

\subsubsection{Navegación GPS}

Los sistemas de navegación modernos emplean variantes de A* (como ALT: A* with Landmarks and Triangle Inequality) para calcular rutas en grafos de millones de nodos \cite{bast2016route}. Zeng y Church \cite{zeng2009finding} demuestran empíricamente que A* con heurística Euclidiana supera a todas las variantes evaluadas de Dijkstra en redes viales reales, estableciéndolo como el algoritmo preferido para consultas de camino más corto punto a punto en sistemas GIS. La arquitectura extensible de \texttt{search-library} permite incorporar estas optimizaciones mediante nuevas heurísticas.
